...
try {
    ...
    if (isset($_SESSION["memNo"])) {
        ...
        switch ($type) {
            case "chat":
                // 聊天室內容
                $sql = "select * from chat where (memNo1 = :memNo1 and memNo2 = :memNo2) or (memNo2 = :memNo1 and memNo1 = :memNo2) order by chat.chatTime";
                ...
                $sql = "select matPosUrl from `matpostmark` where (memNo = :memNo2) and (mugStatus = 1);";
                $matPosUrl = $pdo->prepare($sql);
                $matPosUrl->bindValue(":memNo2", $_POST["memNo2"]);
                $matPosUrl->execute();
                $mem2img = $matPosUrl->fetchAll(PDO::FETCH_ASSOC);
