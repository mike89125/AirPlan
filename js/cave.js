$(document).ready(function () {
  ...
  async function cavAjax(memNo) {
    /* 顯示第一封歷史信件所對應信件標題、內文、圖片 */
    function letterContent(fullLet) {
      ...
      for (let i = 0; i < fullLet.length; i++) {
        if (firstLook == fullLet[i][1]) {
          ...
          $(".letChat").attr("id", "letChat" + fullLet[i][4]);
    ...
    /* 點擊觸發mine 顯示我的歷史信件 手機版*/
    $(".cav-myLetter").on("touchend", function mine() {
      // console.log("我的");
      $.ajax({
        url: "./phps/cav-whosLetter.php",
        type: "POST",
    ...
    $(".cav-otherLetter").on("click", function others() {
      ...
      $.ajax({
        ...
        success: function (letRowOther) {
          ...
          if (letNoChat != memNo) {
            $("#letChat" + letNoChat).attr("disabled", false);
            $(document).click(".letChat", function (e) {
              // let objChat = e.target.parentNode;
              // console.log(objChat);
              // let whoGotChat = e.target.parentNode.dataset.letchat;
              let whoGotChat = e.target.parentNode.id.toString().substring(7);
    ...
    /* AJAX *註冊* 打賞留言跳金幣,並只能打賞一次 手機版*/
    $(document).on("touchend", ".like", function (e) {
      let obj = e.target.parentNode.dataset.like;
      let now = new Date();
      let likeTime = now.getFullYear() + "-" + (now.getMonth() + 1) + "-" + now.getDate() +
        " " + now.getHours() + ":" + now.getMinutes() + ":" + now.getSeconds();
    ...
    /* 打賞信件跳金幣,並只能打賞一次 手機版*/
    $(".letLike").on("touchend", function (e) {
      let obj = e.target.parentNode;
      let likeThis = e.target.parentNode.dataset.letlike;
      let now = new Date();
      let likeTime = now.getFullYear() + "-" + (now.getMonth() + 1) + "-" + now.getDate() +
    ...
    $(".cav-sendReport").click(function (e) {
        ...
        $.ajax({
          ...
          success: function (reportRow) {
            ...
            if (reportRow.status == 'success') {
              $("#report" + reportThis).attr("disabled", true);
              // console.log(obj1);
            } else {
              // console.log(111);
            }
    ...
    /* 點擊不同信件更換顯示信件回復 手機版*/
    $(document).on("touchend", ".cav-letters", function () {
      let witchLetMsg = $(this).find(".cav-letTitle").attr("data-letter-num");
      // console.log(witchLetMsg); 6
      $.ajax({
        url: "./phps/cav-letMessage.php",
  ...
  function cavJs() {
    ...
    //信件分類按鈕 手機板
    $("div.btn").on("touchend", function (e) {
      $("div.btn").removeClass("click");
      $(this).addClass("click");
    });
    ...
    //單一信件跳窗 動態綁定功能 手機板
    $(document).on("touchend", ".cav-letters", function (e) {
      $(".cav-boxFrontRight").addClass("active");
    });
    $(document).on("touchend", ".cav-letClose", function (e) {
      $(".cav-boxFrontRight").removeClass("active");
    });
    ...
    //選取所要套用的圖案 手機板
    $("div.cav-prodI").on("touchend", function () {
      $("div.cav-prodI").removeClass("wearing");
      $(this).addClass("wearing");
    });
    ...
    //選取所要套用的郵戳 手機板
    $("div.cav-prodU").on("touchend", function () {
      $("div.cav-prodU").removeClass("wearing");
      $(this).addClass("wearing");
    });
...
function getCaveInfo(fun) {////////////////////////取得session資料
  let xhr = new XMLHttpRequest();
  xhr.onload = function () {
